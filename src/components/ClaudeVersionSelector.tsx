...
export const ClaudeVersionSelector: React.FC<ClaudeVersionSelectorProps> = ({
  ...
  const loadInstallations = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "加载 Claude 安装路径失败");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    if (simplified) {
      return (
        <div className="space-y-2">
          <Label className="text-sm font-medium">Claude 安装</Label>
    ...
    return (
      <Card className={className}>
        <CardHeader>
          <CardTitle>Claude Code 安装</CardTitle>
          <CardDescription>正在加载可用的安装路径...</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className={cn("space-y-3", className)}>
        ...
        <Select value={selectedInstallation?.path || ""} onValueChange={handleInstallationChange}>
          <SelectTrigger id="claude-installation" className="w-full">
            <SelectValue placeholder="选择 Claude 安装">
              {selectedInstallation && (
