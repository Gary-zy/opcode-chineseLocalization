  ...
  const handleImportFromFile = async () => {
    try {
      ...
      if (filePath) {
        ...
        setToast({ message: `智能体 "${agent.name}" 导入成功`, type: "success" });
      }
    } catch (error) {
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      ...
      if (filePath) {
        ...
        setToast({ message: "智能体导出成功", type: "success" });
      }
    } catch (error) {
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                                  ...
                                  <Badge variant="outline" className="text-xs">
                                    {run.model === 'opus' ? 'Claude 3.5 Opus' : 'Claude 3.5 Sonnet'}
                                  </Badge>
                                </div>
                              </div>
                              <Button
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              您确定要删除“{agentToDelete?.name}”吗？此操作无法撤销。
            </DialogDescription>
          </DialogHeader>
