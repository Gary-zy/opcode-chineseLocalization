...
export const CheckpointSettings: React.FC<CheckpointSettingsProps> = ({
  ...
  const handleCleanup = async () => {
    try {
      ...
      setSuccessMessage(`已移除 ${removed} 个旧检查点`);
      setTimeout(() => setSuccessMessage(null), 3000);
    ...
    >
      ...
      <div className="flex items-center justify-between pb-2 border-b border-border">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-heading-4 font-semibold">检查点设置</h3>
            <p className="text-caption text-muted-foreground mt-0.5">管理会话检查点与恢复</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-md border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/20 p-3">
        <div className="flex items-start gap-2.5">
          ...
          <div className="space-y-0.5">
            <p className="text-caption font-medium text-amber-900 dark:text-amber-100">实验性功能</p>
      ...
      <Card className="p-5 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <Label htmlFor="auto-checkpoint" className="text-label">自动创建检查点</Label>
        ...
        <div className="space-y-2">
          <Label htmlFor="strategy" className="text-label">检查点策略</Label>
          <SelectComponent
          ...
          />
          <p className="text-caption text-muted-foreground">
            {checkpointStrategy === "manual" && "仅手动创建检查点"}
            {checkpointStrategy === "per_prompt" && "每次输入提示词后都会创建检查点"}
            {checkpointStrategy === "per_tool_use" && "每次工具使用后都会创建检查点"}
            {checkpointStrategy === "smart" && "在执行破坏性操作（如编辑文件）后创建检查点"}
          </p>
        </div>
      ...
      <Card className="p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <div className="flex items-center gap-2">
              ...
              <Label className="text-label">存储管理</Label>
            </div>
            <p className="text-caption text-muted-foreground">
              检查点总数：<span className="font-medium text-foreground">{totalCheckpoints}</span>
            </p>
          </div>
        </div>
        ...
        {/* Cleanup settings */}
        <div className="space-y-2">
          <Label htmlFor="keep-count" className="text-label">保留最近的检查点数量</Label>
