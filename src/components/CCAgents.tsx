...
export const CCAgents: React.FC<CCAgentsProps> = ({ onBack, className }) => {
  ...
  const loadAgents = async () => {
    try {
      ...
      setError("加载代理失败");
      setToast({ message: "加载代理失败", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDeleteAgent = async () => {
    ...
    try {
      ...
      setToast({ message: "代理已成功删除", type: "success" });
      await loadAgents();
      await loadRuns(); // Reload runs as they might be affected
    } catch (err) {
      console.error("Failed to delete agent:", err);
      setToast({ message: "删除代理失败", type: "error" });
    } finally {
      setIsDeleting(false);
  ...
  const handleAgentCreated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: "代理已成功创建", type: "success" });
  };

  const handleAgentUpdated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: "代理已成功更新", type: "success" });
  };
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      ...
      setToast({ message: `代理 "${agent.name}" 导出成功`, type: "success" });
    } catch (err) {
      console.error("Failed to export agent:", err);
      setToast({ message: "导出代理失败", type: "error" });
    }
  };
  ...
  const handleImportAgent = async () => {
    try {
      ...
      setToast({ message: "代理导入成功", type: "success" });
      await loadAgents();
    } catch (err) {
    ...
    <div className={cn("flex flex-col h-full bg-background", className)}>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-heading-1">CC 代理</h1>
              ...
              <div>
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <h3 className="text-heading-4 mb-2">暂无代理</h3>
      ...
      <GitHubAgentBrowser
        ...
        onImportSuccess={async () => {
          ...
          await loadAgents();
          setToast({ message: "从 GitHub 导入代理成功", type: "success" });
        }}
      />
