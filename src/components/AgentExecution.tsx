  ...
  const handleBackWithConfirmation = () => {
    if (isRunning) {
      ...
      const shouldLeave = window.confirm(
        "智能体正在运行中。如果您离开，智能体将在后台继续运行。您可以在“CC Agents”的“运行中的会话”选项卡中查看运行中的会话。\n\n您确定要继续吗？"
      );
      if (!shouldLeave) {
        return;
      }
  ...
  const handleCopyAsMarkdown = async () => {
    ...
    for (const msg of messages) {
      if (msg.type === "system" && msg.subtype === "init") {
        ...
        if (msg.message.usage) {
          markdown += `*Token: ${msg.message.usage.input_tokens} 输入, ${msg.message.usage.output_tokens} 输出*\n\n`;
        }
        ...
        if (msg.duration_ms !== undefined) {
          markdown += `- **耗时:** ${(msg.duration_ms / 1000).toFixed(2)}s\n`;
        }
        if (msg.num_turns !== undefined) {
          markdown += `- **轮数:** ${msg.num_turns}\n`;
        }
        if (msg.usage) {
          const total = msg.usage.input_tokens + msg.usage.output_tokens;
          markdown += `- **总 Token:** ${total} (${msg.usage.input_tokens} 输入, ${msg.usage.output_tokens} 输出)\n`;
        }
      }
    }
        ...
        <div className="p-6 border-b border-border">
          <div className="max-w-4xl mx-auto space-y-4">
            ...
            <div className="space-y-3">
              <Label className="text-caption text-muted-foreground">模型选择</Label>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-caption text-muted-foreground">速度更快，效率高</div>
                    </div>
                  </div>
                </motion.button>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-caption text-muted-foreground">能力更强</div>
                    </div>
                  </div>
                </motion.button>
              </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label className="text-caption text-muted-foreground">任务描述</Label>
                {projectPath && (
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  >
                    ...
                    <span className="text-caption">配置 Hooks</span>
                  </Button>
                )}
              </div>
              <div className="flex gap-2">
              ...
              {projectPath && (
                <p className="text-caption text-muted-foreground">
                  工作目录: <span className="font-mono">{projectPath.split('/').pop() || projectPath}</span>
                </p>
              )}
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {isRunning && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-green-600 font-medium">正在运行</span>
                </div>
              )}
            </div>
            <div className="flex items-center gap-2">
      ...
      >
        <DialogContent className="max-w-4xl max-h-[85vh] overflow-hidden flex flex-col gap-0 p-0">
          <div className="px-6 py-4 border-b border-border">
            <DialogTitle className="text-heading-2">配置 Hooks</DialogTitle>
            <DialogDescription className="mt-1 text-body-small text-muted-foreground">
              配置在工具执行之前、期间和之后运行的钩子
            </DialogDescription>
          </div>
          ...
          <Tabs value={activeHooksTab} onValueChange={setActiveHooksTab} className="flex-1 flex flex-col overflow-hidden">
            ...
            <TabsContent value="project" className="flex-1 overflow-auto px-6 pb-6 mt-0">
              <div className="space-y-4 pt-4">
                <div className="rounded-lg bg-muted/50 p-3">
                  <p className="text-caption text-muted-foreground">
                    项目钩子存储在 <code className="font-mono text-xs bg-background px-1.5 py-0.5 rounded">.claude/settings.json</code> 中，
                    并提交至版本控制，允许团队成员共享配置。
                  </p>
                </div>
                <HooksEditor
            ...
            <TabsContent value="local" className="flex-1 overflow-auto px-6 pb-6 mt-0">
              <div className="space-y-4 pt-4">
                <div className="rounded-lg bg-muted/50 p-3">
                  <p className="text-caption text-muted-foreground">
                    本地钩子存储在 <code className="font-mono text-xs bg-background px-1.5 py-0.5 rounded">.claude/settings.local.json</code> 中，
                    且不提交至版本控制，非常适合个人偏好。
                  </p>
                </div>
                <HooksEditor
